...
public class MoveByAction : Action
{
    ...
    public override void Execute()
    {
        ...
        bool gaurdedByAutoInteraction = false;

        foreach (var interaction in _map.Interactions)
        {
            if (interaction.AutoActivated && interaction.Position == newPosition)
            {
                interaction.IsActive = true;
                interaction.ContinueDialog();
                gaurdedByAutoInteraction = true;
            }
        }

        if (!gaurdedByAutoInteraction)
        {
